<template>
  <div id="app">
    <div id="documenteditor_toolbar" style="border: 1px solid #e4e4e4">
      <ejs-toolbar ref="de_toolbar" class="e-de-toolbar" :items="toolbarItems" v-bind:clicked="onToolbarClick" />
    </div>
    <input type="file" id="fileUpload" ref="uploadDocument" style="position:fixed; left:-100em" @change="fileChange" accept=".dotx,.docx,.docm,.dot,.doc,.rtf,.txt,.xml,.sfdt,.json" />
  ...
  mounted() {
    ...
    fileChange: function (args) {
      if (args.target.files[0]) {
        ...
        if (['.sfdt','.json'].includes( path.name.substr(path.name.lastIndexOf('.')))) {
          ...
          ajax.open('POST', 'https://localhost:6001/api/WeatherForecast/Import', true);
